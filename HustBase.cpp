...
void CHustBaseApp::OnCreateDB()
{
	...
	SHGetSpecialFolderLocation(NULL,CSIDL_DRIVES,&rootPath);
	if(rootPath == NULL){
		//printf("");
		return;
	}
	ZeroMemory(&bi,sizeof(BROWSEINFO));
	bi.pszDisplayName = cPathName;
	bi.lpszTitle = cTitle;
	bi.ulFlags = 0x0040;
	bi.pidlRoot = rootPath;

	LPITEMIDLIST idl = SHBrowseForFolder(&bi);
	SHGetPathFromIDList(idl, str.GetBuffer(MAX_PATH * 2));
	str.ReleaseBuffer();
	BOOL flg = CreateDirectory(str, NULL);

	dbPath = str.GetBuffer(0);
	dbName = cPathName;
	RC rc = CreateDB(dbPath,dbName);
	if (rc != SUCCESS) {
		//printf("there must be something goes wrong");
		return;
	}
